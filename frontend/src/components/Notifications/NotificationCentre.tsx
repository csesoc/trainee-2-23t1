...
const NotificationCentre: React.FC<{notificationSee: boolean, setNotificationSee: any, darkMode: boolean}> = (props) => {
  ...
  return (
    <div className={props.darkMode ? "dark" : "light"} ref={notifMenu}>
    <div className="absolute right-64 top-[68px] h-auto z-10 bg-navbar dark:bg-gray-800 flex justify-center items-center text-black p-2 rounded-xl float-right">
        ...
        </div>
      {
        notifs.length === 0 ? <div>&nbsp;You don't have any notifications.&nbsp;</div> : (
          notifs.map(notif => {
            return (
              <Notification darkMode={props.darkMode}/>
            )
          })
        )
      }
