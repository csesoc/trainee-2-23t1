...
const Notification: React.FC<{notifMeta: TNotif, setNotif: any}> = ({ notifMeta, setNotif }) => {
  const { createdBy, tideTitle } = notifMeta

  const accept = trpc.user.acceptTide.useMutation()
    ...
    <div className="flex flex-row pr-10 pl-5 py-2 cursor-pointer hover:bg-gray-100/50 dark:hover:bg-gray-800/50">
      ...
      <div className="flex flex-col justify-around">
        ...
        <div className="flex flex-row items-center justify-end">
          <button onClick={() => {accept.mutate({ tideId: notifMeta.id }); setNotif((n: TNotif[]) => n.filter(not => not.id !== notifMeta.id))}}>
